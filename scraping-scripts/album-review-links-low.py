...
def main():
    aoty_url = "https://www.albumoftheyear.org/ratings/6-highest-rated/2010s/"

    for i in range(1, 200, 2):
        html = fetch_html(aoty_url + str(i))
        album_hrefs = extract_album_links(html)
        with open("review-links-low.txt", "a") as review_links_file:
            for album_href in album_hrefs:
                review_links_file.write("https://www.albumoftheyear.org" + album_href[:-4] + "/user-reviews/?sort=worst\n")
        print(i, "/200 done", sep="")
        time.sleep(5)
